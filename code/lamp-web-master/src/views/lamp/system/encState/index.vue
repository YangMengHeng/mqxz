<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input
        :placeholder="$t('table.encState.nodeIp')"
        class="filter-item search-item"
        v-model="queryParams.model.nodeIp"
        placeholder='节点ip地址'
      />
      <el-input
        :placeholder="$t('table.encState.nodeName')"
        class="filter-item search-item"
        v-model="queryParams.model.nodeName"
        placeholder='	节点名称'
      />
      <el-input
        :placeholder="$t('table.encState.nodePort')"
        class="filter-item search-item"
        v-model="queryParams.model.nodePort"
        placeholder='	节点端口号'
      />
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.encState.nodeIp')" :show-overflow-tooltip="true" align="center"
                       prop="nodeIp" width="" label=' 节点ip地址'>
        <template slot-scope="scope">
          <span>{{ scope.row.nodeIp }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.encState.nodeName')" :show-overflow-tooltip="true" align="center"
                       prop="nodeName" width="" label=' 节点名称'>
        <template slot-scope="scope">
          <span>{{ scope.row.nodeName }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.encState.nodePort')" :show-overflow-tooltip="true" align="center"
                       prop="nodePort" width="" label='节点端口号'>
        <template slot-scope="scope">
          <span>{{ scope.row.nodePort }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.encState.nodeState')" :show-overflow-tooltip="true" align="center"
                       prop="nodeState" width="" label='节点状态'>
        <template slot-scope="scope">
  ...
  export default {
    ...
    methods: {
      ...
      exportExcelPreview() {
        ...
        encStateApi.preview(this.queryParams).then(response => {
      ...
      exportExcel() {
        ...
        this.queryParams.extra.fileName = '导出参数数据';
        encStateApi.export(this.queryParams).then(response => {
      ...
      delete(ids) {
        encStateApi.delete(ids).then(response => {
          const res = response.data;
          if (res.isSuccess) {
            this.$message({
      ...
      fetch(params = {}) {
        ...
        encStateApi.page(this.queryParams).then(response => {
          const res = response.data;
          if (res.isSuccess) {
